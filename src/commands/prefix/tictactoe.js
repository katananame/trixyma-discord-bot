...
module.exports = {
    ...
    async execute(message, args, client) {
        if (!args[0]) {
            return message.channel.send({
                embeds: [createErrorEmbed(
                    ...
                    '❌ Please mention a player to play against.\n\n' +
                    '📋 Usage: `t!tictactoe @player`\n' +
                    '💭 Example: `t!tictactoe @User`',
                    client
                )]
            });
        }
        ...
        if (!opponent) {
            return message.channel.send({
                embeds: [createErrorEmbed('Please mention a valid user to play against!', client)]
            });
        }

        if (opponent.bot) {
            return message.channel.send({
                embeds: [createErrorEmbed('You cannot play against a bot!', client)]
            });
        }

        if (opponent.id === challenger.id) {
            return message.channel.send({
                embeds: [createErrorEmbed('You cannot play against yourself!', client)]
            });
        }
        ...
        if (activeGames.has(challenger.id) || activeGames.has(opponent.id)) {
            return message.channel.send({
                embeds: [createErrorEmbed('One of the players is already in a game!', client)]
            });
        }
